...
### 索引与存储（重构）
    - **脚本物理文件平铺化 + 无扩展名**：所有脚本文件一律以「`{id}`」（UUID、无扩展名）命名、平铺在 script 目录下（与 index.json 同级），不再分子目录；索引只保留层级语义（children），重命名 / 拖拽移动层级等后续操作只改 JSON、不动物理文件。存量 34 个脚本已迁移改名、13 个空子目录已清除（原目录与索引备份于 `D:\.Backup\knife-script-manager-flat-20260915-154514`）。
    - AI 生成脚本落盘到 script 根目录（`{id}` 无扩展名），`ai-generated/` 隔离目录随之取消；物理文件不含扩展名，**解释器完全由索引条目的 `lang` 字段决定**，与文件名无关（运行期程序会把脚本另存为带正确扩展名的临时文件再执行，语言识别由 `lang` 保证）。
    - 物理文件无扩展名，故**改语言 / 内容只改 index.json 的 `lang` 字段与文件内容，无需改文件名**；`path` 固定为 `./<id>`。
    - **（2026-09-15）脚本文件去除扩展名**：存量 34 个 `{id}.{ext}` 脚本已重命名为 `{id}`、index.json 的 34 条 `path` 由 `./<id>.<ext>` 改为 `./<id>`；`ScriptIndexStore.FileExtensionForLang` 与「切换语言跟随改扩展名」逻辑已移除。改语言现在仅改 `lang` 字段、文件内容可随意改写，文件名恒定。备份见 `D:\.Backup\knife-script-manager-extless-20260915-175220`。
- AI 多轮对话历史持久化到缓存：每个脚本一个以脚本 id 命名的文件夹（`cache/{脚本id}/`），每次编辑会话一个文件（会话内每轮成功后覆盖更新）；删除脚本不清除对应缓存。

### 界面与交互
- **输入框文字与占位符精确对齐（全局修复）**：所有输入框的实际输入文字此前都比占位符偏右 / 偏下——根因是 TextBox 自定义模板把 `Padding` 同时绑到了 `PART_ContentHost(ScrollViewer)` 的 `Margin` 上，而 `TextBoxBase` 内部本就会把 `Padding` 应用到文本视图，**双重叠加**（实测落点 = 2×Padding + 3px 固有内缩）。修复：删除 `BaseTextBox` / `ClearableTextBoxInline` / 主窗口路径框三处模板的 `Margin={Padding}` 绑定，只保留内建机制（落点 = Padding + 3px），并按「文字落点 = 占位符 Margin」重校各框内边距——单行框 Padding 1（落点 4 = 占位符 4）、描述框 Padding 6,7（落点 9,8 = 占位符 9,8）、执行路径框 Padding 5（落点 8 = 占位符 8）。已用独立探针程序在真实窗口逐框实测：文字与占位符偏差 (0,0)。
- **脚本树右键菜单**：在目录树区域按节点类型动态弹出右键菜单，删除操作均带二次确认（MessageBox，默认「否」）：
  ...
  - `创建脚本（AI）` 打开 AI 编辑器（创建模式）：脚本文件以 `{id}`（无扩展名）落 `script/` 根目录，索引条目插入所点目录节点下，写入后自动刷新树。
