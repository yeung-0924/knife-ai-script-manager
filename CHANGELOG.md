...
### 新增功能
- **配置编辑器分两个分区 + AI「最大对话轮次」**：设置 ▸ 编辑配置拆为「工作路径」（脚本索引 / 各目录选择 / 默认执行超时）与「模型配置」（API 密钥 / 地址 / 模型 / 最大对话轮次）两个带标题的分区。新增 `[ai] max_rounds`：一次编辑会话中与 AI 对话的轮数上限（**含首轮生成**），默认 1 = 仅首轮生成、无追问；达到上限后「生成」按钮禁用、状态栏提示收尾，生成失败不计入轮数，「接受并写入」后轮次归零重新起算（保存即生效，无需重启）。
