...
| `type` | 否 | 输入控件类型：`text`（默认，文本框）/ `select`（下拉，需配合 `options`）/ `file`（文本框 + 浏览…按钮，选文件）/ `folder`（文本框 + 浏览…按钮，选目录）。**仅影响界面呈现**，取值仍按原样代入占位符 |
| `open_after_run` | 否 | `true` 时，脚本**执行成功（退出码 0）后**自动在资源管理器打开本参数指向的路径：目录参数打开该目录，文件参数打开所在目录并选中该文件。典型用途是导出类脚本的「导出目录」（如 `爬虫 ▸ 中国行政区划`）。失败 / 超时 / 被停止时不打开；参数留空或路径不存在则静默跳过 |
