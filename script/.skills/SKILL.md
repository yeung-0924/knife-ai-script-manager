...
- **可选：跑完自动打开输出目录**——给参数加 `"open_after_run": true`，脚本**成功结束（退出码 0）后**程序会自动在资源管理器打开该参数指向的路径（目录 → 打开该目录；文件 → 打开所在目录并选中该文件）。导出类脚本的「导出目录」加上它，用户跑完就能直接看到结果；失败 / 超时 / 被停止时不打开，参数留空或路径不存在则静默跳过。
